...
Homesteading the Noosphere: Japanese#link("https://validator.w3.org/check/referer")[]

#title[ノウアスフィアの開墾]

（Homesteading the Noosphere）

#link("http://www.catb.org/~esr/")[Eric S. Raymond] 著\
#link("mailto:hiyori13@alum.mit.edu")[山形浩生 YAMAGATA Hiroo 訳]　　　　リンク、コピーは*黙って*どうぞ。#link("https://cruel.org/linkpolicy.html")[くわしくはこちらを見よ。]

プロジェクト杉田玄白　正式参加作品。詳細は #link("http://www.genpaku.org/")[http://www.genpaku.org/] を参照のこと。

　1998年7月11日、1998年7月18日翻訳完了、2000年5月1日更新

原文の最新版は#link("http://www.catb.org/~esr/writings/homesteading/")[http://www.catb.org/~esr/writings/homesteading/]にて各種フォーマットで入手可能。\
翻訳の pdf 版は#link("https://cruel.org/freeware/noosphere.pdf")[https://cruel.org/freeware/noosphere.pdf ]にある。\
翻訳の PostScript+tar+gzip 版は#link("https://cruel.org/freeware/noosphere.tgz")[https://cruel.org/freeware/noosphere.tgz ]にある。\
\
前編 *「伽藍とバザール」 (The Cathedral and the Bazaar)* も是非参照されたし。\
原文は#link("http://www.catb.org/~esr/writings/cathedral-bazaar/")[http://www.catb.org/~esr/writings/cathedral-bazaar/]にて各種フォーマットで、\
翻訳は #link("https://cruel.org/freeware/cathedral.html")[https://cruel.org/freeware/cathedral.html] にて入手可能。

第 3 部 *「魔法のおなべ」 (The Magic Cauldron)* も是非参照されたし。\
原文は#link("http://www.catb.org/~esr/writings/magic-cauldron/")[http://www.catb.org/~esr/writings/magic-cauldron/]にて各種フォーマットで、\
翻訳は #link("https://cruel.org/freeware/magicpot.html")[https://cruel.org/freeware/magicpot.html]にて入手可能。\
\
...
#link("#1")[1　そもそもの矛盾]\
\
#link("#2")[2　ハッカーイデオロギーのさまざま]\
\
#link("#3")[3　放縦な理論と純潔な実践]\
\
#link("#4")[4　所有権とオープンソース]\
\
#link("#5")[5　ロックと土地所有権]\
\
#link("#6")[6　贈与経済としてのハッカー文化]\
\
#link("#7")[7　ハッキングのよろこび]\
\
#link("#8")[8　評判のさまざまな相貌]\
\
#link("#9")[9　所有権と評判によるインセンティブ]\
\
#link("#10")[10　エゴの問題]\
\
#link("#11")[11　謙虚さの美徳]\
\
#link("#12")[12　評判ゲームモデルが持つ分野全体としての意義]\
\
#link("#13")[13　どれほどすばらしい贈り物？]\
\
#link("#14")[14　ノウアスフィア的所有権となわばりの動物行動学]\
\
#link("#15")[15　紛争の原因]\
\
#link("#16")[16　プロジェクト構造と所有権]\
\
#link("#17")[17　紛争とその解決]\
\
#link("#18")[18　文化への順応過程とアカデミズムとの関連]\
\
#link("#19")[19　結論：慣習から慣習法へ]\
\
#link("#20")[20　これからの研究の課題]\
\
#link("#21")[21　書誌、注]\
\
#link("#22")[22　謝辞]\
...
　何年ものあいだ、FSF はオープンソース・ハッキングに唯一最大の焦点だったし、いまだにこの文化にとってきわめて重要なツールを生み出し続けてきている。FSF はまた、ハッカー文化を外から見ている者にとっては、ながいことオープンソースを支援している組織的な身分をもった唯一のスポンサーだった。かれらこそ実質的に「フリーソフト」という用語を定義し、このことばに意図的に挑戦的な含みを持たせるようになった。（新しいラベルの「#link("http://www.opensource.org/")[オープンソース]」は同じくらい意図的にそういう含みをさけるようにしている。）
...
　プラグマティストたちにとって、GPL はそれ自体が目的なんじゃなくて、ツールとして重要だ。その主な価値は「秘匿」に対する武器としてのものじゃない。むしろソフトの共有と#link("https://cruel.org/freeware/cathedral.html")[バザール様式]の開発コミュニティ成長を奨励するためのツールとして大事になる。プラグマティストは、商業主義を嫌うよりはいいツールやおもちゃを手に入れるほうを重視するし、商業製品を使う場合でも、それが高品質なら別にイデオロギー上の不快感は感じない。同時に、オープンソース界での経験でおぼえた技術的な品質は、閉鎖的なソフトではほとんど実現困難なほど高い。
...
　1997年に、こういう共通要素が蒸留されて Debian フリーソフトウェア・ガイドラインとなり、これが#link("http://www.opensource.org/")[Open Source Definition]になった。OSD の定義したガイドラインのもとでは、オープンソースのライセンスはオープンソース・ソフトを変更（そしてその変更版を再配布）する権利をだれにでも無条件に認めなくてはならない。
...
　でも実際には、こんな「分裂」はほとんど絶対に起きないといっていい。大プロジェクトの分裂はすごくまれだし、いつも名前が変わるうえ、世間的にもなぜそれが正当かについて、大量の説明が行われる。GNU Emacs/Xemacs の分裂や、さまざまな BSD 系グループの分裂なんかでは、分離派たちは自分たちが非常に強力なコミュニティ規範に逆らって行動していると感じていたのは明らかだ \[#link("#SP")[SP]\]。
...
- ある人の名前をプロジェクトの歴史やクレジットや管理者リストからのぞくのは、当人のはっきりした合意なしには*絶対に行われない*。
...
　実はオープンソース文化に関するかぎり、この質問には簡単に答えられる。ソフトウェア・プロジェクトの所有者というのは、*変更したバージョンを公式に再配布する*独占的な権利をコミュニティ全般から認められている人物である。
...
　標準的なオープンソースのライセンスのもとなら、ソフト進化のゲームではみんなが平等なはずだ。でも現実には、公認の管理者が承認して進化するソフトに統合した「公式」パッチと、第三者による「非公式」パッチとははっきり区別して認識されている。非公式パッチはあまりないし、一般には信用のおけないものとされる#link("#RP")[\[RP\]]。

　*公式な*再配布こそが根本的な問題なんだというのは、簡単に示せる。ハッカー文化の慣習は、みんなが自分だけのためにソフトをパッチするのは推奨している。この習慣は、閉じたユーザグループや開発グループ内で変更バージョンを再配布する人々も問題にはしない。そういう変更がオープンソース・コミュニティ一般に対してポストされ、オリジナル版と競合するようになったときにだけ、所有権が問題になってくる。

　一般的にいって、オープンソース・プロジェクトの所有権を獲得するには3つの方法がある。第一の方法はいちばん自明だろうけれど、プロジェクトを創始することだ。プロジェクトの開始以来、管理者が一人しかいなくて、その管理者が活動を続けているなら、ハッカー慣習はそのプロジェクトをだれが所有しているのかについて*疑問視することすら*許さない。
...
　入植済みの地域での土地移譲は、ふつうは*土地所有権の移転*によって行われる。これはつまり、前の所有者から証書を受け取ることだ。この理論に基づけば、所有権の連鎖という概念が重要になる。所有権の証明として理想的なのは、証書とその移転の連鎖が、その土地のそもそもの開墾時点にまでさかのぼれることだ。

　最後に、慣習法理論は土地の所有権が失われたり放棄されたりすることがあるのを認識している（たとえば所有者が相続人なしに死んだり、空き地に対する所有権の連鎖を確立するための記録が失われていたりする場合）。このようにして遺棄された土地は、*占拠*によって所有権の主張を行える――だれかがそこに入居して、整備を行い、そして開墾時と同じようにそこの所有権を防衛するわけだ。
...
　論理的によく似た理論は、ある物件が経済的または生存上で高い価値を持っていて、しかも稀少財の配分について、中央集権的に強制するだけの力を持った単一の権威が存在しないところでは必ず生じるようだ。これは、ときどきロマンチックに「所有」の概念がないと考えられたりもする狩猟採集文化でも成り立つ。たとえば Kgalagadi (カラハリ) 砂漠のクンサン・ブッシュマンの伝統では、狩猟場には所有権は存在しない。でも、*井戸や泉については*所有権が存在して、その理論は明らかにロックの理論と似ている。
...
　この論文の題名に出てくる「ノウアスフィア（noosphere）」というのはアイデア（観念）の領域であり、あらゆる可能な思考の空間だ#link("#N")[\[N\]]。ハッカーの所有権慣習に暗黙に含まれているのは、ノウアスフィアの部分集合の一つであるすべてのプログラムを包含する空間での、所有権に関するロック理論なんだ。だからこの論文は「ノウアスフィアの開墾」と名付けた。新しいオープンソース・プロジェクトの創始者がみんなやっているのがそれだからだ。

　Fare Rideau （rideau\@ens.fr）}は、ハッカーは純粋観念の領域だけで活動しているとはいえないと指摘している。これは正しい。かれの主張だと、ハッカーが所有しているのは*プログラミング・プロジェクト*なんだ―― これはつまり、物質的な労働（開発、整備など）の意図的な焦点で、ここに評判、信用性なんかが結びついている。ここからかれは、ハッカーのプロジェクトが覆っている空間はノウアスフィア*ではなく*、むしろその分身のような、ノウアスフィアを探索するプログラム・プロジェクトだと主張する（読者のなかの天文物理学者たちに敬意を表しつつも、語源的にいえばこの分身空間を「エルゴスフィア」または「作業のスフィア」と呼んでまちがいない）。

　現実問題としては、ノウアスフィアとエルゴスフィアとの区別はこの論文の目的からいえばだいじじゃない。Fare が主張するような純粋な意味での「ノウアスフィア」が、なんら意味あるかたちで存在すると言っていいのかは、実はよくわからない。そんなものを信じるには、プラトン主義哲学者にでもなるしかないだろう。そしてノウアスフィアとエルゴスフィアとの区別が*実際的な*重要性を持つのは、アイデア・観念（ノウアスフィアの要素）は所有できずそれをプロジェクトとして実体化したものだけが所有できると考えたい場合だけだ。この問題から出てくる課題は、知的所有権の理論的な問題につながるもので、この論文の範囲を超える。\[#link("#DF")[DF]\]
...
　でも、こういう経済的な副作用が処理されるやり方は検討する価値がある。これから、オープンソース文化*そのもの*のなかでの評判の力学だけでも、かなりの説明力があることを見てみよう。\
\

= 6　贈与経済としてのハッカー文化

\
　オープンソース文化での評判の役割を理解するには、歴史から離れて文化人類学と経済学に深入りし、*交換の文化*と*贈与の文化*のちがいを検討してみると役に立つ。
...
　一番簡単な方法は *上意下達方式（command hierarchy）*だ。上意下達方式では、稀少な財の配分は一つの中央権力が行って、それが軍事力でバックアップされる。上意下達方式は、規模の変化への適応力（スケーラビリティ）がものすごくとぼしい#link("#Mal")[\[Mal\]]。大きくなるにつれて、ますます横暴で非効率になってゆく。このため、大家族以上の上意下達方式はほぼかならずといっていいほど、別のかたちのもっと大きな経済に寄生する存在でしかない。上意下達方式では、社会的地位はおもに恐喝力へのアクセス能力によって決まってくる。

　ぼくたちの社会はもっぱら*交換経済*だ。これは財の希少性に対する洗練された適応方式で、規模の変化にもよく適応する。稀少な財の配分は、交換と自発的な協力によって非中心的に行われる（そして実は、競争の欲望がもたらす最大の効果は協力行動を生み出すことだ）。交換経済では、社会的地位はおもにもの（必ずしも物質的なものとは限らない）のコントロールの大小で決まる。

　ほとんどの人は、この二つについては説明されるまでもなく精神的なモデルを持っているし、それらがどう相互に機能するかもわかっている。政府や軍、ギャング集団などは、ぼくたちが「自由市場」とよぶもっと大きな交換経済に寄生している上意下達システムだ。しかしながら、このどちらともまったくちがっていて、人類学者たち以外はあまり認知されていない第三のモデルがあるんだ。これが*贈与の文化*だ。
...
　過剰は上意下達関係を維持困難にして、交換による関係をほとんど無意味なゲームにしてしまう。贈与の文化では、社会的なステータスはその人がなにをコントロールしているかではなく、*その人がなにをあげてしまうか*で決まる。
...
　でもこの観察は、それだけではハッカー文化に見られる特徴を説明するのに十分とはいえない。クラッカー d00dz は、ハッカーと同じ（電子）メディア上に息づく贈与文化を持っているけれど、その行動は大きくちがっている。このグループの性行は、ハッカーよりもずっと強くて排外的だ。共有するよりは秘密を隠匿したがる。クラッカーグループでは、ソフトクラック用のソースのない実行ファイルが出回ることのほうが、そのやりかたを教えるヒントが出回ることより多い。（この行動についての、d00dz集団内部からの観点については \[#link("#LW")[LW]\] を参照。）

　これが示しているのは、もちろん言うまでもなく自明だろうけれど、贈与文化の運用方法は一つじゃないってことだ。ぼくはハッカー文化の歴史について、別のところでまとめたことがある#link("#HH")[\[HH\]]。それがいまの行動を形成していった方法は、謎なんかじゃない。ハッカーたちは、自分たちの文化を規定するにあたり、自分たちの競合が行われる*形式*の集合を利用したわけだ。この論文ではこの先、その形式を検討することにしよう。\
\
...
　とはいっても、職人気質モデルは捨てないでおこう。それは多くのハッカーにとって直感的にわかりやすいし、個人行動のある面はすごくうまく説明してくれるから。#link("#HT")[\[HT\]]
...
　ほかの反応としては、同業者の中での名誉という報酬とハッキングの喜びが、生活維持に必要な水準以上になっているということを、人間の動機づけに関するアイブラハム・マズローの有名な「価値のヒエラルキー」モデルと関連づけている（#link("#MH")[\[MH\]]）。この観点からすると、ハッキングの喜びというのは自己実現ないし高次ニーズであって、低次のニーズ（たとえば肉体的な安定性や「帰属感」や同業者の中の名誉など）が最低限は満たされない限り、一貫性を持った形で表明されることはないのだ、ということになる。だから評判ゲームは、ハッキングの喜びが*まさに*個人の主要動機となれるような社会的文脈を提供するにあたって、とても重要なものとなるかもしれない。 \
\
...
　第二に、名声は他人の注目を集めて協力を得るのにすごく有効な方法だ（そして純粋な贈与経済では、*唯一無二*の方法だ）。もしある人が、気前よく、知的で、公正で、指導力があるとかいったよい資質で有名だったら、その人と関わりを持つことでメリットがあると他人に説得するのはずっと簡単になる。
...
　この理解にもとづいて、ぼくたちはハッカー社会のロック流の所有権慣習が*評判上のインセンティブを最大化する*手段なんだと分析できる。つまり、仲間内のクレジットがしかるべき人物に帰属して、それ以外のところには行かないようにするわけだ。
...
- 非公式パッチを配布する（あるいはもっとひどい場合には非公式バイナリを配布する）と、所有者たちは不公正な評判上のリスクにさらされる。公式コードが完璧であっても、所有者たちはパッチのバグのおかげで不評をかうはめになりかねない（とはいえ、#link("#RP")[\[RP\]]を参照のこと）。
...
　非公式パッチへの批判は、それがバグ追跡をものすごく難しくすることや、*自前の*バグを処理するだけでも手いっぱいな管理者の作業を、さらに増やす点などを観察したうえで説明されることが多い。
...
　同じようなノイズに対する脆弱さは、ハッカーコミュニティの部族の長老たちに要求される、公的な場での謙虚さも説明してくれる。長老たちは、傲慢さや尊大さに冒されていないと見られなくてはならない。そうしないと、危険なノイズに対するタブーが維持されないからだ。#link("#DC")[\[DC\]]
...
　アプリケーションだ。2000 年が近づくにつれて、オープンソース開発がますます最後の処女地に向かってシフトすると予言してもよさそうだ。その処女地とはすなわち、技術オンチのためのプログラムだ。はっきりした最初のしるしとしては、#link("http://www.gimp.org/")[GIMP]の開発がある。これはフォトショップに似た画像処理ソフトで、過去 10 年には商業アプリケーションの独壇場と思われていた、エンドユーザにやさしい GUI を持っている。別の示唆としては、アプリケーション用ツールキットのプロジェクトを取り巻いているにぎやかさが指摘できるだろう。#link("http://www.kde.org/")[KDE] や #link("http://www.gnome.org/")[GNOME] なんかだ。

　この論文の評者の一人は、開墾のアナロジーを使うことで、ハッカーたちがなぜマイクロソフトの「抱き込んで拡張」方針、つまりインターネットプロトコルを複雑化して閉じたものにしてしまおうという方針に対し、なぜあれほど露骨な怒りをもって反応するのか、ということが説明できることを指摘している。ハッカー文化はほとんどのクローズドなソフトと共存できる。たとえば Adobe Photoshop があっても、そのオープンソース版とも言うべき GIMP 周辺の領域が極端に魅力がなくなるわけではない。でも、マイクロソフトがプロトコルを「脱・共有化」#link("#HD")[\[HD\]]してしまって、マイクロソフト自身のプログラマしかそれを使うソフトが書けないようにしてしまったら、マイクロソフトは単に独占を拡大して顧客に害を与えるだけではすまない。ハッカーたちが開墾して耕作すべきノウアスフィアの量と質を低下させることにもなる。ハッカーたちがマイクロソフトの戦略を「プロトコル汚染」と呼ぶのも当然だろう。かれらは、自分の農業用水に毒を入れようとしている人を見つけた農夫たちとまったく同じ反応を示しているわけだ！

　最後に、評判ゲーム分析はよく挙げられる格言を説明してくれる。人はハッカーを名乗ればハッカーになれるわけじゃない――*ほかのハッカーに*ハッカーと呼ばれた時点で、人はハッカーになるんだ。この観点から見た「ハッカー」というのは、技術的な能力を持っていて、しかも評判ゲームがどう機能するかわかっているということを（贈り物を提供することで）実証した人間だ。この判断は、おもに認知と文化順応に基づくもので、すでにこの文化の中にしっかり入った人々からしか与えられない。\
\
...
　こいつをもっと素朴に表現するなら、「独創的な作業は、既存ソフトの機能を複製するよりもいい」ということになるだろう。でも、実はこんなに単純ではない。既存の*クローズドな*ソフトの機能を複製するのは、オリジナルなソフトの開発作業と同じくらい高い評価が得られる――それによって、クローズドなプロトコルやフォーマットをこじあけたりして、その領土を新たに提供できるのであれば。
...
　職人モデルは、問題（B）「だれがなにについてクレジットを与えられるか」についてのハッカー慣習を説明するには十分ではない（だって、純粋な職人は評判ゲームなんか気にしないから、そんなことを気にする動機がない）。これを分析するには、ロック理論をもう一歩進めて、所有権上の抗争や運用がどう行われているかを*プロジェクト同士*だけでなく、*プロジェクト内部*から見てやらなくてはならない。\
\
...
　紛争は原則的に、こうした二つの基準（「権威は責任にともなう」「序列の高いほうが勝つ」）が矛盾する結果を出して、*しかも*そのプロジェクトリーダの権利が弱かったり不在だったりするときにしか生き延びない。これが起きるいちばんはっきりした例は、プロジェクトのリーダが消えたあとで、その後継をめぐる争いだ。ぼくは一度、この種の争いに加わっていたことがある。みにくくて苦痛で、長引いて、最終的に解決されたのも、関係者がみんな疲れ切って、コントロールを外部の人に任せようと決めたから可能になった。もう二度とあの種のものには近づかずにすみますように、と心から願わずにはいられない。
...
　この点は、もし本当なら、ただの（失礼）学問的興味以上のものとなる。それは『伽藍とバザール』（#link("./cathedral.html")[邦訳]、#link("http://www.catb.org/~esr/writings/cathedral-bazaar/")[原文])での考察を、ちょっと別の角度からも論証しなおしたものとなる。つまり多くのプログラマがポスト稀少性時代の贈与文化の中で生きられるだけの富の余剰を資本主義が作り出せるようになった瞬間から、最終的にはソフト生産の産業資本主義様式は競争に負けるべく運命づけられているのだ、という考察だ。 \
\
...
　英米慣習法の伝統の土地所有権理論が、ハッカー慣習と論理的にとても近い関係にあることを見てきた。歴史的にみて #link("#Miller")[\[Miller\]]、これを発明したヨーロッパの部族文化は、明文化もはっきり意識化もされない慣習から部族の賢者たちが記憶するはっきりした慣習法体系に移行し、やがてはそれを成文化することで、紛争解決システムを向上させていった。
...
　ぼくはそういうコードを書く作業を始めていて、ぼくが住んでいる小さな町の名前をとってそれを仮に「マルヴァーン綱領」と名付けている。もしこの論文の一般的な分析がそれなりに広く受け入れられたら、マルヴァーン綱領を紛争解決コードの見本として公開する。このコードの批評や開発に興味のある向きや、単にそれがいいか悪いかフィードバックをくれたい人は、電子メールで#link("mailto:esr@thyrsus.com")[ぼく]にコンタクトしてほしい。\
\

= 20　これからの研究の課題

\
　ハッカー文化（そしてぼく）は、優しい独裁者モデルに従わない大規模プロジェクトについてあまりよく理解できていない。そういうプロジェクトの多くは失敗する。でも少数はものすごい大成功をおさめて重要になる（Perl、Apache、KDE）。ちがいがどこにあるのか、だれにもわかっていない。（そういうプロジェクトはそれぞれsui generis（まったく独自のもの）であり、たまたまそのメンバーのグループ力学に基づいて成功したりしなかったりするのだというのが、漠然とただよう理解だけれど、でもこれって本当なんだろうか、それともある集団がとるべき複製可能な戦略があるんだろうか？）\
\
...
すごく軽い冗談口てんこもりだけれど、そのなかの「SNAFU 原理」なる代物は、上意下達方式がなぜスケーラビリティが低いのかについて、なかなか深遠な分析を提供している。#link("http://www.cs.cmu.edu/~tilt/principia/")[ブラウズ可能な HTML 版]あり。
...
\[MHG\] Goldhaber, Michael K.; *#link("http://www.firstmonday.dk/issues/issue2_4/goldhaber")[The Attention Economy and the Net]*\
...
\[HH\] ハッカー社会（hackerdom）の歴史については、#link("http://earthspace.net/~esr/faqs/hacker-hist.html")[ここ]にまとめてみた（#link("./hackerdom.html")[邦訳はこちら]）。これをほんとうにきちんと説明してくれる本は未だに書かれていないし、それを書くのはたぶんぼくではないと思う。

\[N\]「ノウアスフィア（noosphere）」は哲学特有の専門用語で、「*ノウ*・ア・スフィア」が正しい発音（訳注：ティヤール・ド・シャルダンが使っていて、日本ではヌースフィアと書かれることが多い。この翻訳では著者にしたがった）。表記について死ぬほど厳密でありたいなら、正しくは二番目の o の上に点々がつく。
...
\[DF\]現代経済学で最も雄弁かつわかりやすい論者デビッド・フリードマンは、知的財産法の歴史と論理について#link("http://www.best.com/~ddfr/Academic/Course_Pages/L_and_E_LS_98/Why_Is_Law/Why_Is_Law_Chapter_11.html")[見事な概論]を書いている。この問題に関心ある人すべての出発点としておすすめする。

\[SP\]　Linux と BSD の世界とのおもしろいちがいの一つは、Linux のカーネル（および関連する OS の中心的なユーティリティ）は一度も分裂していないけれど、BSD は少なくとも 3 回は分裂しているということだ。これがおもしろいのは、BSD グループの社会構造は中央集権化されていて、権限をはっきりと線引きすることで分裂を防ぐようになっているのに、中央集権化されていなくて不定型なLinuxコミュニティは分裂を防ぐような手段をまったく講じていないからだ。開発をなるべく開放したほうが、いちばん分裂しにくくなるとしか思えない！

Henry Spencer \<#link("mailto:henry@spsystems.net")[henry\@spsystems.net]\> の考えによれば、一般に政治的なシステムの安定性は、その政治プロセスへの参入障壁の高さに反比例するのではないか、とのこと。かれの分析は、ここで引用するだけの価値がある：
...
（同じような原理は経済でも見られる。開放市場では競合がいちばん激しく、さらに最高で最低価格の製品が得られるのがふつうだ。このため、既存企業としては、新規参入をなるべくむずかしくするのがいちばん利益になる――たとえば政府に対し、コンピュータに入念な RFI テストを義務づけさせたり、ポケットが深くない限り、複雑すぎて1から実装するのが実質的に不可能な「合意に基づく」規格を作ったりするわけだ。参入障壁がいちばん強力な市場は、革命家、たとえばインターネットや、司法省対ベルシステムの独禁法裁判などからいちばん攻撃を受けやすい市場でもある。）
...
\[RP\]非公式パッチについてはいくつか細かい点がある。まずこれは、「友好的なもの」と「敵対するもの」に分類できる。「友好的」なパッチは管理者のコントロール下で、プロジェクトのソースコードの主流に織り込まれるように設計されている（それが実際に織り込まれるかはまた別の話）。「敵対」パッチは、管理者の認めない方向にプロジェクトをねじまげようとするものだ。一部のプロジェクト（とくに Linux カーネルそのもの）は友好パッチにはすごく甘くて、むしろベータテストの段階では、独自のパッケージを推奨したりさえする。一方の敵対パッチはオリジナルと張り合おうという決断を示すもので、深刻な事態となる。敵対パッチを大量に管理するようになると、プロジェクトはやがて分裂に向かうことが多い。

\[LW\]ハッカーと海賊ソフト文化との対比がいかに示唆的かを指摘してくれた点で、Michael Funk \<mwfunk\@uncc.campus.mci.net\>には感謝したい。Linus Wallejは、かれらの文化力学についてぼくとは異なる分析をポストしている（かれらを希少性文化として描いている。これは *A Comment on \`Warez D00dz' Culture* (#link("http://www.df.lth.se/~triad/papers/Raymond_D00dz.html")[http://www.df.lth.se/~triad/papers/Raymond\_D00dz.html]を参照。

　この対比は、長続きしないかもしれない。もとクラッカーのAndrej Brandt \<andy\@pilgrim.cs.net.pl\> の見解によると、cracker/warez d00dz 文化は現在しぼみつつあって、いちばん優れた人材やリーダーたちは、オープンソースの世界に取り込まれているという。この視点について独立した証拠としては、「Cult of the Dead Cow」というクラッカー集団が 1999 年 6 月に行った、前例のない行動がある。かれらは、Microsoft Windows のセキュリティを破るためのツールである「Back Orifice 2000」を GPL のもとで公開した。

\[HT\]進化論的にいえば、職人の衝動自体が（内面化された倫理と同じく）裏切りの高リスクとコストの結果かもしれない。進化心理学者たちは、人間が社会的なだましを検出するのに特化した脳論理回路を持っているという証拠を得ている#link("#BCT")[\[BCT\]]。そして、われわれの先祖がインチキを検出する能力を持ったために選別されたのはなぜか、理解するのは容易だろう。だから、もしリスクもコストも高いけれど、メリットももたらすような性格の持ち主だという評判を得たければ、そういう性質があるふりをしてごまかすよりも、実際にそういう性質を身につけるほうが戦術的に優れているかも知れない（「正直がいちばん」というやつだ）。
...
\[MH\] マスローのヒエラルキーと関連理論についての簡潔なまとめは、web上で読める。「Maslow's Hierarchy of Needs」（#link("http://www.valdosta.peachnet.edu/~whuitt/psy702/regsys/maslow.html")[http://www.valdosta.peachnet.edu/~whuitt/psy702/regsys/maslow.html]）

\[DC\] しかしながら、リーダーに謙虚さを求めるというのは、贈与文化、過剰文化のもっと一般的な性質なのかもしれない。David Christie \<dc\@netscape.com\> は、フィジーの周辺部の島を旅したときについてこう書いている:

「フィジーの村長にも、あなたがオープンソースのリーダーについて述べたのと同じように、卑下する腰の低いリーダーシップのスタイルが観察されました。 \[中略\] 大いなる尊敬を集め、そしてフィジーにある限りの現実の力をすべて持っているのに、われわれが会った村長たちは本物の謙虚さを示し、しばしば自分たちの任務を聖人のように受け入れていました。これは、村長というのが世襲だということを考えるとなおさら興味深いものです。選出されるわけでもないし、人気投票で決まるのでもありません。どういうわけか、生まれついてのことで仲間に選ばれたわけでもないのに、文化そのものによって訓練されているらしいのです。」

　かれはさらに、このフィジーの村長に特有のスタイルは、協力を強制するのがむずかしいという事実からくるのだと思っていることを強調している。村長は「大きなニンジンも、大きな棒も持っていない」わけだ。

\[NO\]見ればすぐにわかることだが、うまくいったプロジェクトを創始した人のほうが、成功したプロジェクトのデバッグや援助で同じくらいの仕事をした人にくらべて、高い評判を獲得する。この論文のかつてのバージョンでは、次のような疑問を提示していた：「これは比較できる作業の価値評価として合理的なものだろうか、それともぼくたちがここで示してきた無意識の領土モデルからくる二次的な副産物なんだろうか？」　何人かのコメントは、説得力ある、基本的には同じ理論を示唆してくれている。以下の Ryan Waldron \<rew\@erebor.com\> の分析がこの論点をうまく表現している：
...
　でも、新しいプロジェクトが \[だれか別の人によって\] 明日にでも創始されたら、そしてぼくが初期の熱心な参加者になったら、そのうまくいったプロジェクトによって生じた尊敬の分け前をもらう能力は、早めに参加していたということによって大きく拡大する（貢献の中身が同程度だったとしても）。たぶん、早めに Micro\$oft 株を買った人と、後で投資した人の差に近いと思う。みんな儲かるけれど、早めに買った人はもっと儲かる。だからどこかの時点で、ぼくは既存の企業の株が絶えず値上がりするのに加わるよりは、新しい成功する IPO のほうに興味をおぼえるようになるはずだ。」

Ryan Waldron のアナロジーはもっと拡張できる。プロジェクトの創始者は、他の人に受け入れられるか、つかいものになるかもわからない新しいアイデアを、伝道師のように売り込まなくてはならない。だから創始者は IPO リスクと似たよなものを負担することになる（自分の評判へのダメージの可能性として）。これは、仲間内である程度受け入れられたプロジェクトを手伝った人よりも、負担するリスクが大きい。創始者の見返りは、実際問題として助手たちが仕事をたくさんつぎ込んでいても、一定だ。これは交換経済におけるリスクと見返りの関係として楽に理解できる。

　他の評者たちの見解では、われわれの神経システムは、ちがいを認識するように調整されていて、安定状態には敏感ではない。新しいプロジェクトの創造によって目的される革命的な変化は、したがってたえまない段階的な改善の集積効果より、ずっと認知されやすい。だから Linus は Linux の父として尊敬される。その他何千という貢献者たちによる貢献の純粋量は、Linus 一人の仕事なんかでは絶対に不可能なほど、OS の成功に貢献してはいても。

\[HD\] 「脱・共有化」（de-commoditizing）ということばは、「#link("./halloween.html")[ハロウィーン文書]」への言及。この文書でマイクロソフトは、顧客を収奪できるような独占的囲い込みを維持するための、いちばん有効な長期戦略として、この「脱共有化」を何のためらいもなく提案している。

\[GNU\]　フリーソフトウェア財団 (Free Software Foundation) の主ウェブサイトは、#link("http://www.gnu.org/philosophy/motivation.html")[こうした研究の結果をまとめた記事]
...
Robert Lanphiir \<robla\@real.com\> はエゴのない行動についての議論に大きく貢献してくれた。Eric Kidd \<eric.kidd\@pobox.com\> は、個人崇拝を避ける意味で謙虚さが大事であるという点を明確にしてくれた。全世界的な影響についての章は、Daniel Burn \<daniel\@tsathoggua.lab.usyd.edu.au\> からのコメントに触発された。Mike Whitaker \<mrw\@entropic.co.uk\> は文化への順応に関する議論の大筋の発想を与えてくれた。Chris Phoenix \<cphoenix\@best.com\> は、ハッカーたちは他のハッカーを追い落としても評判を高めることはできないという事実の重要性を指摘してくれた。\
\
...
#link("https://validator.w3.org/check/referer")[]YAMAGATA Hiroo \<hiyori13\@alum.mit.edu\>
